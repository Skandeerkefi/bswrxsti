...
import { useSlotChallengeStore, ChallengeResult } from '@/store/useSlotChallengeStore';
...
export default function SlotChallengeDetailPage() {
  ...
  const fetchLeaderboardData = async (challengeId: string) => {
  ...
  const sortedLeaderboard = leaderboardData?.leaderboard || 
    (currentChallenge?.leaderboard ? [...currentChallenge.leaderboard].sort((a, b) => 
            ...
            <div className="divide-y divide-[#D2758F]/10">
              {sortedLeaderboard.length === 0 ? (
                ...
                sortedLeaderboard.map((entry, index) => {
                  ...
                  const meetsTarget = entry.highestMultiplier.multiplier >= currentChallenge.targetMultiplier;
                    ...
                    <div
                      ...
                      <div className="flex items-center col-span-4 gap-3 md:col-span-3">
                        <div className="flex items-center justify-center w-10 h-10 overflow-hidden bg-[#D2758F]/20 rounded-full">
                          {entry.rankLevelImage ? <img src={entry.rankLevelImage} alt="" className="object-cover w-full h-full" /> : <User className="w-5 h-5 text-[#D2758F]" />}
                        </div>
                        <div>
                          <p className="font-medium text-[#FEFDDE] truncate">{entry.username}</p>
                          {entry.rankLevel > 0 && <p className="text-xs text-[#FEFDDE]/40">Level {entry.rankLevel}</p>}
                        </div>
                      </div>
                      <div className="col-span-3 text-right md:col-span-2">
                        <p className={`text-xl font-bold ${meetsTarget ? 'text-green-400' : 'text-[#FEFDDE]'}`}>{entry.highestMultiplier.multiplier.toLocaleString()}x</p>
                      </div>
                      <div className="hidden col-span-2 text-right md:block">
                        <p className="text-[#FEFDDE]/80">${entry.highestMultiplier.wagered.toFixed(2)}</p>
                      </div>
                      <div className="hidden col-span-2 text-right md:block">
                        <p className="text-green-400">${entry.highestMultiplier.payout.toFixed(2)}</p>
                      </div>
                      <div className="col-span-3 text-right">
                        {isWinner ? (
                          ...
                          <span className="inline-flex items-center gap-1 px-3 py-1 text-sm font-medium text-green-400 border rounded-full bg-green-500/20 border-green-500/50">Qualified</span>
                        ) : (
                          <span className="inline-flex items-center gap-1 px-3 py-1 text-sm font-medium text-[#FEFDDE]/60 border rounded-full bg-[#D2758F]/10 border-[#D2758F]/30">Participating</span>
                        )}
                      </div>
                    </div>
                  );
